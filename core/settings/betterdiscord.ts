...
export function getSetting(id: string, defaultValue: any) {
    if (!id.includes(".")) return BdApi.loadData(manifest.id, id) ?? defaultValue;

    const items = id.split(".");
    const first = items.shift();

    return items.reduce((curr, key) => curr?.[key], BdApi.loadData(manifest.id, first)) ?? defaultValue;
}

export function setSetting(id: string, newValue: any) {
    if (!id.includes(".")) return (BdApi.saveData(manifest.id, id, newValue), emitChange(id));

    const items = id.split(".");
    const first = items.shift();
    const value = BdApi.loadData(manifest.id, first) ?? {};
    ...
    BdApi.saveData(manifest.id, first, value);
    emitChange(id);
}
